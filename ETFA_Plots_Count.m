...
figure (3);
subplot(2,1,1);
mean_count_FD = cat(1,mean(OTM_F_Count_FDBT),mean(OTM_N_Count_FDBT),...
                  mean(OTO_F_Count_FDBT),mean(OTO_N_Count_FDBT));
stderror_count_FD = cat(1,std(OTM_F_Count_FDBT)/sqrt(length(OTM_F_Count_FDBT)),...
                      std(OTM_N_Count_FDBT)/sqrt(length(OTM_N_Count_FDBT)),...
                      std(OTM_N_Count_FDBT)/sqrt(length(OTM_N_Count_FDBT)),...
                      std(OTO_N_Count_FDBT)/sqrt(length(OTO_N_Count_FDBT)));
  
  errorbar(mean_count_FD,stderror_count_FD,'rx'); 
  ylim([0 20]);
  ylabel('Count','FontSize',10);
  set(gca,'XTick',[1:4],'FontSize',7);
  set(gca,'xticklabel',{'OTM-Failure', 'OTM-Normal','OTO-Failure', 'OTO-Normal'},'FontSize',10);
   title('Std error plot: First Sample beyond threshold','FontSize',12);

 subplot(2,1,2);
mean_count_NT = cat(1,mean(OTM_F_Count_NoPtAT),mean(OTM_N_Count_NoPtAT),...
                  mean(OTO_F_Count_NoPtAT),mean(OTO_N_Count_NoPtAT));
stderror_count_NT = cat(1,std(OTM_F_Count_NoPtAT)/sqrt(length(OTM_F_Count_NoPtAT)),...
                      std(OTM_N_Count_NoPtAT)/sqrt(length(OTM_N_Count_NoPtAT)),...
                      std(OTM_N_Count_NoPtAT)/sqrt(length(OTM_N_Count_NoPtAT)),...
                      std(OTO_N_Count_NoPtAT)/sqrt(length(OTO_N_Count_NoPtAT)));
  
  errorbar(mean_count_NT,stderror_count_NT,'rx'); 
  ylim([0 20]);
  ylabel('Count','FontSize',10);
  set(gca,'XTick',[1:4],'FontSize',7);
  set(gca,'xticklabel',{'OTM-Failure', 'OTM-Normal','OTO-Failure', 'OTO-Normal'},'FontSize',10);
   title('Std error plot : Number of samples above threshold','FontSize',12);  
 set(gcf,'color','w'); 
