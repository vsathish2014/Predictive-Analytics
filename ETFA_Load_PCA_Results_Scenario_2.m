...
for file = files'
    ...
    dist_data_All_Test = readtable(file.name,'Sheet',3,'Range','A1:DM21'); 
    ...
    diff_data_All_Test = readtable(file.name,'Sheet',5,'Range','A1:DM21'); 
    ...
    data_All_Test = readtable(file.name,'Sheet',7,'Range','A1:HZ21'); 
...
%     model = pca(training,noPCs,options);
   ...
   elements_above_threshold(:,i) =  cat(1,find(q_residual_pred(:,i)>q_confLimit(:,i)), zeros(20-size_elements_above_threshold(1,1),1));
    ...
    t_squared_prediction(1:20,i) = prediction.tsqs{1,1};
...
test_results = cat(2,Avg_residual',Max_residual',Std_residual',first_pt_q_residual_norm',...
                   first_pt_outside_threshold',  count_elements_above_threshold',...
                      Avg_tsq',Max_tsq',Std_tsq');
