...
% Forward dynamics - with a robot model (no friction)
    ...
    % Add Measurement noise to Q, QD, and QDD
       ...
       %clear y1,y2,y3,y4,y5,y6;
        ...
        y1 = QD(:,1) + nl'.*0.0001*10 ;
         y2 = QD(:,2) + nl'.*0.0001*10 ;
         y3 = QD(:,3) + nl'.*0.0001*10 ;
         y4 = QD(:,4) + nl'.*0.0001*10 ;
         y5 = QD(:,5) + nl'.*0.0001*10 ;
         y6 = QD(:,6) + nl'.*0.0001*10 ;

        QD_mn(:,:) = cat(2,y1,y2,y3,y4,y5,y6);
        
        %clear y1,y2,y3,y4,y5,y6;
        
         y1 = QDD(:,1) + nl'.*0.0001*100 ;
         y2 = QDD(:,2) + nl'.*0.0001*100 ;
         y3 = QDD(:,3) + nl'.*0.0001*100 ;
         y4 = QDD(:,4) + nl'.*0.0001*100 ;
         y5 = QDD(:,5) + nl'.*0.0001*100 ;
         y6 = QDD(:,6) + nl'.*0.0001*100 ;
